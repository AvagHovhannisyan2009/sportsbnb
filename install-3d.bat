@echo off
REM 🚀 3D Immersive Website - Installation Script for Windows
REM Run this script to install all required dependencies

echo 🎨 Installing 3D Immersive Website Dependencies...
echo.

REM Install main 3D packages with compatible versions for React 18
echo 📦 Installing Three.js and React Three Fiber...
call npm install --legacy-peer-deps three@0.169.0 @react-three/fiber@8.17.10 @react-three/drei@9.114.3 @react-three/postprocessing@2.16.3

echo.
echo 🎬 Installing Animation Libraries...
call npm install --legacy-peer-deps gsap@3.12.5 @studio-freight/lenis@1.0.42

echo.
echo 🔢 Installing Math Utilities...
call npm install --legacy-peer-deps maath@0.10.8
